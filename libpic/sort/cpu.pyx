...
cdef void calculate_cell_index(
    double* x, double* y, cnp.npy_bool* pruned, 
...
def _calculate_cell_index(
    ...
    calculate_cell_index(
        <double*>cnp.PyArray_DATA(x), <double*>cnp.PyArray_DATA(y), <cnp.npy_bool*>cnp.PyArray_DATA(pruned),
        npart,
        nx, ny, dx, dy,
        x0, y0,
...
cdef intp cycle_sort(
    ...
    intp nx, intp ny, 
    intp* particle_cell_indices, cnp.npy_bool* pruned, intp* sort_idx
) noexcept nogil:
    """
...
# python wrapper
def _cycle_sort(cell_bound_min, cell_bound_max, nx, ny, particle_cell_indices, pruned, sort_idx):
    return cycle_sort(
        <intp*> cnp.PyArray_DATA(cell_bound_min), 
        <intp*> cnp.PyArray_DATA(cell_bound_max), 
        nx, ny, 
        <intp*> cnp.PyArray_DATA(particle_cell_indices), 
        <cnp.npy_bool*> cnp.PyArray_DATA(pruned), 
...
def sort_particles_patches(
    ...
    intp npatches,
    CListIntp particle_cell_indices_list, CListIntp sorted_indices_list, CListDouble x_list, CListDouble y_list, CListBool pruned_list,
    ...
    for ipatch in prange(npatches, nogil=True, schedule='runtime'):
        ...
        cycle_sort(cell_bound_min, cell_bound_max, nx, ny, particle_cell_indices, pruned, sorted_indices)
